...
class _CoinRewardOverlayState extends State<CoinRewardOverlay> {
  ...
  /// All unlocked items, newest first (newest = leftmost in the row).
  List<({String char, int index})> _unlockedItems() =>
      CollectiblesState.unlockedInOrder().reversed.toList();
        ...
        child: SingleChildScrollView(
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                // Coin count or friendly zero-coins message
                if (widget.coins == 0)
                ...
                _SectionArea(
                  ...
                  child: _CollectibleRow(
                    items: unlocked,
                    state: _CardState.unlocked,
                ...
                if (affordable.isEmpty && unaffordable.isEmpty)
                  ...
                  _LockedGrid(
                    ...
                    onUnlock: (char, index) =>
                        setState(() => _justUnlocked = (char: char, index: index)),
                  ),
...
class _LockedGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final all = [
      for (final item in affordable)
        (char: item.char, index: item.index, state: _CardState.affordable),
      for (final item in unaffordable)
        (char: item.char, index: item.index, state: _CardState.locked),
    ];
          ...
          for (final item in items)
            Padding(
              ...
              child: _CollectibleCard(
                ...
                index: item.index,
                state: state,
                isNew: justUnlocked?.char == item.char &&
                    justUnlocked?.index == item.index,
              ),
            ),
        ],
      ),
...
class _CollectibleCardState extends State<_CollectibleCard>
  ...
  Widget build(BuildContext context) {
    ...
    final asset =
        'assets/images/${_CollectibleCard._assetPrefix[widget.char]}_l${widget.index + 1}.png';
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        final borderWidth =
            isUnlocked && widget.isNew ? 2.0 + (1.0 - t) * 2.0 : 2.0;
                      ...
                      Positioned(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            border:
                                Border.all(color: LaraColors.galletaBrown, width: 1),
                          ),
                          child: Icon(
                            isAffordable
