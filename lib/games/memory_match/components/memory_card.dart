...
class MemoryCard extends SpriteComponent with TapCallbacks, HasGameReference<MemoryMatchGame> {
  MemoryCard({required this.back, required this.face, required this.faceId, required this.col, required this.row})
    : super(sprite: back);
  ...
  void markMatched() {
    ...
    add(
      ScaleEffect.by(
        Vector2.all(1.18),
        EffectController(duration: 0.22, alternate: true, reverseDuration: 0.22, curve: Curves.easeOutBack),
      ),
    );
    add(
      ScaleEffect.by(
  ...
  void render(Canvas canvas) {
    ...
    final rect = RRect.fromRectAndRadius(Rect.fromLTWH(0, 0, size.x, size.y), radius);
    ...
    const p = padding;
    sprite?.render(canvas, position: Vector2.all(p), size: Vector2(size.x - p * 2, size.y - p * 2));
  }
