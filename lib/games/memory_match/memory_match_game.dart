...
class MemoryMatchGame extends LaraBaseGame {
  ...
  List<String> _candidateNames() {
    ...
    for (var i = 0; i < CollectiblesState.count; i++) {
      pool.add('collectibles/rhenne/rhenne_l${i + 1}.png');
      excludeFallbacks.add('rhenne.png');

      pool.add('collectibles/galleta/galleta_l${i + 1}.png');
      excludeFallbacks.add('galleta.png');

      pool.add('collectibles/heart/heart_l${i + 1}.png');
      excludeFallbacks.add('corazon.png');
    }
    for (var i = 0; i < QrUnlockState.count; i++) {
      pool.add('collectibles/lara/lara_d${i + 1}.png');
  ...
  void _buildBoard() {
    ...
    for (var r = 0; r < _rows; r++) {
      for (var c = 0; c < _cols; c++) {
        ...
        final card = MemoryCard(back: _back, face: _faces[faceId]!, faceId: faceId, col: c, row: r)
          ..size = Vector2.all(cardSize)
          ..anchor = Anchor.center
          ..position = Vector2(startX + c * (cardSize + 12), startY + r * (cardSize + 12));
        add(card);
      }
    }
    _built = true;
  ...
  void _reflowBoard() {
    ...
    for (final card in children.whereType<MemoryCard>()) {
      ...
      card.position = Vector2(startX + card.col * (cardSize + 12), startY + card.row * (cardSize + 12));
    }
  }
  ...
  Future<void> onCardTapped(MemoryCard card) async {
    ...
    if (first.faceId == card.faceId) {
      ...
      final mid = Vector2((first.position.x + card.position.x) / 2, (first.position.y + card.position.y) / 2);
      add(_ScorePopup(mid));
      ...
      if (_matches == _pairsThisRound) {
        ...
        showGameOver(
          message: '¡Encontraste a todos los amigos!\nMovimientos: $_moves',
          onRestart: () {},
          onHome: () {},
        );
      }
...
class _DifficultyControls extends PositionComponent with HasGameReference<MemoryMatchGame> {
  ...
  @override
  Future<void> onLoad() async {
    size = Vector2(_labels.length * _pillW + (_labels.length - 1) * _gap, _pillH);
    for (var i = 0; i < _labels.length; i++) {
      add(
        _DifficultyPill(label: _labels[i], index: i)..position = Vector2(i * (_pillW + _gap) + _pillW / 2, _pillH / 2),
      );
    }
  }
...
class _DifficultyPill extends PositionComponent with HasGameReference<MemoryMatchGame>, TapCallbacks {
  _DifficultyPill({required this.label, required this.index})
    : super(size: Vector2(_DifficultyControls._pillW, _DifficultyControls._pillH), anchor: Anchor.center);
  ...
  static final _inactiveText = TextPaint(
    style: const TextStyle(color: Color(0xFF1E1E28), fontSize: 13, fontWeight: FontWeight.w700),
  );

  @override
  void render(Canvas canvas) {
    final isActive = game.difficultyIndex == index;
    final bgPaint = Paint()..color = isActive ? LaraColors.magenta : Colors.white.withValues(alpha: 0.65);
    final rr = RRect.fromRectAndRadius(Rect.fromLTWH(0, 0, size.x, size.y), const Radius.circular(18));
    ...
    (isActive ? _activeText : _inactiveText).render(canvas, label, size / 2, anchor: Anchor.center);
  }
    ...
    : super(
        ...
        textRenderer: TextPaint(
          style: const TextStyle(
            ...
            shadows: [Shadow(color: LaraColors.magenta, blurRadius: 8, offset: Offset(1, 2))],
          ),
        ),
      );
